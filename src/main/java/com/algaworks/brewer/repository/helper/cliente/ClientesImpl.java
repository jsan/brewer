...
public class ClientesImpl implements ClientesQueries {
	...
	public Cliente buscarComCidadeEstado(Long codigo) {
		Criteria criteria = manager.unwrap(Session.class).createCriteria(Cliente.class); // find na Usuario
		criteria.createAlias("endereco.cidade", "c", JoinType.LEFT_OUTER_JOIN);
		criteria.createAlias("c.estado", "e", JoinType.LEFT_OUTER_JOIN);
	...
	private long totalRegs(ClienteFilter filtro) {
		Criteria criteria = manager.unwrap(Session.class).createCriteria(Cliente.class);
		adicionarFiltro(filtro, criteria);
	...
	private void adicionarFiltro(ClienteFilter filtro, Criteria criteria) {
		if (filtro != null) {
			...
			if (!StringUtils.isEmpty(filtro.getCpfOuCnpj())) {
				criteria.add(Restrictions.ilike("cpfOuCnpj", filtro.getCpfOuCnpjSemFormatar(), MatchMode.ANYWHERE));
			}
